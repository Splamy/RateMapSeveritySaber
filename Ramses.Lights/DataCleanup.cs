...
public class DataCleanup
{
	...
	public static void Run(DirectoryInfo mapsDir)
	{
			...
			dir =>
		{
			...
			if (song.Info.CustomData?["_editors"]?.AsObject().Select(x => x.Key).Any(x => string.Equals(x, "beatsage", StringComparison.OrdinalIgnoreCase)) == true)
			{
			...
			foreach (var map in song.Difficulties)
			{
				try
				{
					using var file = provider.Get(map.Difficulty.BeatmapFilename);
					var json = JsonSerializer.Deserialize<JsonObject>(file)!;
					var data = new MapData(map.Difficulty.BeatmapFilename, json);
